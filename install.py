...
subprocess.check_call(shellstr,shell=True)
rosdep_list_custom = "/etc/ros/rosdep/sources.list.d/10-custom.list"
if os.path.exists(rosdep_list_custom):
    shellstr = "sudo rm " + rosdep_list_custom
    subprocess.check_call(shellstr,shell=True)
if not LOCAL:
    shellstr = 'echo "yaml https://bitbucket.org/api/1.0/repositories/peterpolidoro/faa/raw/tip/rosdep.yaml" | sudo tee ' + rosdep_list_custom
else:
    rosdep_yaml = 'yaml file://' + os.path.join(os.environ['FAA_NAME'],'rosdep.yaml')
...
# Setup virtual env
virtualenv = os.environ['FAA_PYTHON_VIRTUALENV']
